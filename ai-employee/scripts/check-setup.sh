...
if [ -f ".mcp.json" ]; then
  ...
  if grep -q "outlook" .mcp.json 2>/dev/null || grep -q "MS_CLIENT_ID" .mcp.json 2>/dev/null; then
    ok "  → Outlook (Microsoft) server configured"
  else
    warn "  → Outlook server NOT in .mcp.json"
  fi
